...
def show_time_detached(user_data: list, title: str= 'coding time'):
    """
    ...
    for user in user_data:
        x.append(user['code_time'])
        x2.append(user['debug_time'])
        if user['code_time'] > max_time:
            max_time = user['code_time']
        if user['debug_time'] > max_time:
            max_time = user['debug_time']

    coding_trace = go.Histogram(x=x, histnorm='count', name='code time', xbins=dict(start=0, end=max_time, size=10))
    debug_trace = go.Histogram(x=x2, histnorm='count', name='debug time', xbins=dict(start=0, end=max_time, size=10))
...
def show_time_total(user_data: list, title: str= 'total coding time'):
    """
    统计不同编码、调试时间的人数。可以得到整体与每题的编码时间人数分布。
    横轴为时间段（min），纵轴为人数（个）
    分为总体情况与每题目情况
    ...
    max_time = 0
    for user in user_data:
        t = user['code_time']+user['debug_time']
        x.append(t)
        if t > max_time:
            max_time = t
...
def show_time_perproblem(user_data: list, userid: str=None, problem_list: list=[], title: str=None):
    ...
    data = {}
    for user in user_data:
        if user['userid'] != userid and userid:
            continue
        if user['problemid'] not in data.keys():
            data[user['problemid']] = {'code_time': user['code_time'], 'debug_time': user['debug_time']}
        else:
            data[user['problemid']]['code_time'] += user['code_time']
            data[user['problemid']]['debug_time'] += user['debug_time']

    for problem in problem_list:
        if problem not in data.keys():
            data[problem] = {'code_time': 0, 'debug_time': 0}
    ...
    for da in data:
        x.append(da[0])
        y.append(da[1]['code_time'])
        y2.append(da[1]['debug_time'])

    if not title:
        if not userid:
            userid = 'all'
        title = userid + ' working time'
    coding_trace = go.Bar(x=x, y=y, name='code time')
    debug_trace = go.Bar(x=x, y=y2, name='debug time')
...
def show_time_div_preproblem(user_data: list, problem_list:list = [], title: str = 'average time ratio'):
    ...
    for user in user_data:
        if user['problemid'] not in data.keys():
            data[user['problemid']] = {'average': user['code_time']/user['debug_time'], 'count': 1}
        else:
            data[user['problemid']]['average'] += user['code_time']/user['debug_time']
            data[user['problemid']]['count'] += 1

    for problem in problem_list:
        if problem not in data.keys():
            data[problem] = {'average': 0, 'count': 1}
    ...
    for da in data:
        x.append(da[0])
        y.append(da[1]['average']/da[1]['count'])
